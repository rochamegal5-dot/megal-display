    ...
    async function cargar() {
      try {
        ...
        const cacheNuevo: TombolaCache = {
          ...
          fechaVespertina:
            nuevasVespertina.length > 0
              ? (json.fechaVespertina ?? json.fecha)
              : cacheActual?.fechaVespertina ?? '',
          fechaNocturna:
            nuevasNocturna.length > 0
              ? (json.fechaNocturna ?? json.fecha)
        ...
        if (cacheNuevo.vespertina.length > 0 || cacheNuevo.nocturna.length > 0) {
          ...
          if (activo) {
            setData({
              ...json,
              fechaVespertina: cacheNuevo.fechaVespertina,
              fechaNocturna: cacheNuevo.fechaNocturna,
    ...
    <div className="panel panel-tombola">
      ...
      <div className="panel-body tombola-panel-body">
        <section className="tombola-section">
          ...
          {data && <small className="result-date">{data.fechaVespertina || data.fecha || 'Último resultado disponible'}</small>}
        ...
        <section className="tombola-section">
          ...
          {data && <small className="result-date">{data.fechaNocturna || data.fecha || 'Último resultado disponible'}</small>}
          <div className="tombola-grid">
            {nocturna.length > 0 ? (
